                ...
                <div class="container wow rotateInDownLeft" data-wow-delay="0.35s" style="max-width: 500px;visibility: visible;
                    ...
                    <div class="row">
                        <div class="col-lg-12 col-md-8 col-sm-8 col-xs-8">
                            <form action="processAddFreedom_Fighters_Sector.php" method="post" enctype="multipart/form-data">
                                <?php
                                ...
                                $stid = oci_parse($conn, 'SELECT * FROM sector');
                                ...
                                while ($row = oci_fetch_array($stid, OCI_ASSOC + OCI_RETURN_NULLS))
                                {
                                    $temp[$var1++] = $row['SECTOR_NO'];
                                }
                                oci_close($conn);
                                ?>
                                <div class="form-group">
                                    <label for="fullname">SELECT SECTOR_NO : </label>
                                    <div class="input-group">
                                            <span class="input-group-addon"> <i
                                                    class="glyphicon glyphicon-user"></i></span>
                                        <div class="icon-addon">
                                            <select name="sector_no" class="form-control" style="height: 30px;">
                                                <?php
                                                for ($p = 0; $p < $var1; $p++)
                                                {
                                                    echo '<option>' . " {$temp[$p]} " . '</option>';
