...
if (!empty($_POST))
{
    ...
    $forceType         = htmlspecialchars($_POST['forceType']);
    $forceDivision             =htmlspecialchars($_POST['forceDivision']);



    $stmt = 'INSERT INTO FORCES_DIVISION
        (FORCES_TYPE,FORCE_DIVISION_NAME)
        VALUES(:FORCES_TYPE,:FORCE_DIVISION_NAME)';

    $stid = oci_parse($c1, $stmt);

    oci_bind_by_name($stid, ':FORCES_TYPE', $forceType);
    oci_bind_by_name($stid, ':FORCE_DIVISION_NAME', $forceDivision);
