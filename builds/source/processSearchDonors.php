...
<?php
$inputValue = strtolower(htmlspecialchars($_GET['searchDonor']));
...
$stid = oci_parse($conn, "SELECT DONORS.DONOR_NAME,
 DONORS.DONATION_DATE,DONORS.DONATION_AMMOUNT
 FROM DONORS WHERE DONORS.DONOR_NAME
LIKE '%" . $_GET['searchDonor'] . "%' ");
...
echo '<thead>';
echo '<tr>';
echo '<th class="active">Donor Name</th>';
echo '<th class="active">Donation Date</th>';
echo '<th class="active">Donation Amount</th>';
echo '</tr>';
echo '</thead>';
echo '<tbody>';
